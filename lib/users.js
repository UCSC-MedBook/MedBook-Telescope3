getEmails=function() {
    return getEmailsFor(Meteor.user());
}
getEmailsForId=function(id) {
    return getEmailsFor(Meteor.users.findOne({_id:id}));
}
getEmailsFor=function(u) {
    var res = [];
    if (u.emails)
        _.map(u.emails, function(a) { res.push(a.address)})
    if (u.services && u.services.google && u.services.google.email)
         res.push(u.services.google.email);
    if (res.length == 0)
        return null;
    return res;
}
...
getEmail = function(user){
    ...
    if (user.emails & user.emails.length > 0 && users.emails[0].address)
        return user.emails[0].address;

    return null;
  /*
     This was telescope's 
